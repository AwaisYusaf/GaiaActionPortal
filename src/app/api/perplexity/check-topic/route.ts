...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      "Sending topic check request to Perplexity API with query:",
      query
    );
...
5. DO NOT include markdown code blocks or any other formatting - just the raw JSON object`,
    ...
    if (data.choices && data.choices[0] && data.choices[0].message) {
      try {
        ...
        try {
          ...
          console.log(
            "Successfully parsed JSON, isEnvironmentalTopic:",
          ...
          if (possibleJsonMatch && possibleJsonMatch[0]) {
            try {
              ...
              console.log(
                "Successfully parsed extracted JSON, isEnvironmentalTopic:",
                isEnvironmentalTopic
              );
            } catch (e) {
              ...
              isEnvironmentalTopic =
                cleanedContent
                  .toLowerCase()
                  .includes('"isenvironmentaltopic": true') ||
                cleanedContent
                  .toLowerCase()
                  .includes('"isenvironmentaltopic":true');
              console.log(
                "Using string matching fallback, isEnvironmentalTopic:",
                isEnvironmentalTopic
              );
            }
          } else {
    ...
    console.log(
      "Final determination - isEnvironmentalTopic:",
      isEnvironmentalTopic
    );
