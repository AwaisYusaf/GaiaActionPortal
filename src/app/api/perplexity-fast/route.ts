  ...
  try {
    ...
    const requestBody = {
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an assistant that generates follow-up questions for environmental research. Be concise and specific.'
        },
        {
          role: 'user',
    ...
    if (!response.ok) {
      ...
      console.error('Perplexity API error (fast model):', response.status, errorText);
