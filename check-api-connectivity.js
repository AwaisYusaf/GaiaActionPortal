...
async function checkApiEndpoint() {
  ...
  try {
    ...
    console.log(
      `Perplexity website response: ${websiteResponse.status} ${websiteResponse.statusText}`
    );

    if (!websiteResponse.ok) {
      console.error(
        "❌ Cannot reach Perplexity website. This may indicate network connectivity issues."
      );
      exit(1);
    }
    ...
    console.log(
      `Perplexity API endpoint response: ${apiResponse.status} ${apiResponse.statusText}`
    );
    ...
    if (
      ...
      console.log(
        "✅ Successfully connected to Perplexity API endpoint (authentication required)\n"
      );
    } else if (!apiResponse.ok) {
      console.error(
        "❌ Cannot reach Perplexity API endpoint. This may indicate network connectivity issues."
      );
      exit(1);
    } else {
    ...
    console.log(
      `API authentication response: ${authResponse.status} ${authResponse.statusText}`
    );
    ...
    if (authResponse.status === 401) {
      ...
      console.log(
        "1. Verify the API key in your .env.local file is correct and up-to-date"
      );
      console.log(
        "2. Check if your Perplexity API key has expired or been revoked"
      );
      console.log("3. Generate a new API key from the Perplexity dashboard");
      console.log(
        "4. Make sure you have sufficient credits in your Perplexity account"
      );
      ...
      console.log(
        "✅ API endpoint reached (Method Not Allowed is expected for HEAD request)"
      );
    } else if (!authResponse.ok) {
      console.error(
        `❌ API endpoint returned unexpected status: ${authResponse.status}`
      );
      exit(1);
    } else {
